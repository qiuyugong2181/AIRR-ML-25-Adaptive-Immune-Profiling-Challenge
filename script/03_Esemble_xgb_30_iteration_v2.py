...
def fit_xgb(X, y, random_state=123, n_iter=150, n_jobs=-1):
    ...
    fixed_params = dict(
        ...
        tree_method="hist",
        random_state=random_state,
        n_jobs=n_jobs, # Let XGBoost handle the multiprocessing
        device="cpu",
        early_stopping_rounds=20 # APPLIED FIX #3: Stop building trees if AUC doesn't improve
    )
    ...
    def objective(trial):
        ...
        # APPLIED FIX #3: Manual CV loop to allow passing eval_set for early stopping
        fold_scores = []
        for train_idx, val_idx in cv.split(X.values, y_arr):
    ...
    # APPLIED FIX #1: n_jobs=1 prevents Optuna from thrashing threads with XGBoost
    study.optimize(objective, n_trials=n_iter, n_jobs=1, show_progress_bar=True)

    best_params = study.best_params
    
    # Remove early stopping for the final model fits so cross_val_score doesn't throw an error
    final_params = fixed_params.copy()
    final_params.pop("early_stopping_rounds", None)
    
    best_model = XGBClassifier(**final_params, **best_params)
...
def main():
    ...
    for train_dir, test_dirs in pairs:
        ...
        X_train_norm = normalize_kmer_rows_by_category(X_train).astype(np.float32)
        vj_train = vj_train.astype(np.float32)
        ...
        for test_dir in test_dirs:
            ...
            X_test_norm = normalize_kmer_rows_by_category(X_test_kmer).astype(np.float32)
            vj_test = vj_test.astype(np.float32)
